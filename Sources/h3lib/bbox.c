...
void scaleBBox(BBox *bbox, double scale) {
    ...
    if (bbox->north > __M_PI_2_2) bbox->north = __M_PI_2_2;
    bbox->south -= heightBuffer;
    if (bbox->south < -__M_PI_2_2) bbox->south = -__M_PI_2_2;
    // Scale east and west, clamping to longitude domain
    bbox->east += widthBuffer;
    if (bbox->east > _M_PI_2) bbox->east -= M_2PI;
    if (bbox->east < -_M_PI_2) bbox->east += M_2PI;
    bbox->west -= widthBuffer;
    if (bbox->west > _M_PI_2) bbox->west -= M_2PI;
    if (bbox->west < -_M_PI_2) bbox->west += M_2PI;
}
